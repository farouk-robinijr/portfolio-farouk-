...
function addLights() {
  scene.add(new THREE.AmbientLight(0xb6b0a8, 0.2));
  scene.add(new THREE.HemisphereLight(0x40536d, 0x100707, 0.58));

  const key = new THREE.DirectionalLight(0xffffff, 4.35);
  ...
  const coolRim = new THREE.DirectionalLight(0x9bc7ff, 1.18);
  coolRim.position.set(-14, 8, -16);
  scene.add(coolRim);

  const redWash = new THREE.PointLight(0xff2525, 1.62, 32, 1.9);
  redWash.position.set(0, 3.4, 1.2);
  scene.add(redWash);

  const lowFill = new THREE.PointLight(0xffffff, 0.72, 28, 2);
...
function addBackground() {
  const skySphere = new THREE.Mesh(
    new THREE.SphereGeometry(92, 96, 48),
  ...
  const stars = new THREE.Points(
    geometry,
    new THREE.PointsMaterial({ size: 0.045, sizeAttenuation: true, vertexColors: true, transparent: true, opacity: 0.58, fog: false }),
  );
  scene.add(stars);
...
function createGalaxyTexture() {
  ...
  drawPlanet(ctx, 70, 1800, 530, "rgba(96,170,255,0.92)", true);
  drawPlanet(ctx, 3520, 420, 150, "rgba(78,150,255,0.72)", false);
  drawPlanet(ctx, 3640, 560, 34, "rgba(136,175,230,0.42)", false);
  drawPlanet(ctx, 1020, 1690, 68, "rgba(130,178,240,0.48)", false);
...
function addBase() {
  ...
  [
    ...
    const marker = new THREE.PointLight(0xff2424, 0.36, 2.4, 2.1);
    marker.position.set(x, y + 0.22, z);
    scene.add(marker);
  });
}
...
function addRoads() {
  ...
  sections.forEach((section) => {
    if (section.id !== "projects") {
      ...
      scene.add(createCurvedRoad(curve([center, bend, section.position.clone().setY(0.04)], false), 0.88, { segments: 22, dashEvery: 5 }));
    }
  });

  scene.add(createCurvedRoad(curve(vehicleRouteAPoints(0.04), true), 0.78, { segments: 112, dashEvery: 8, opacity: 0.88 }));
  scene.add(createCurvedRoad(curve(vehicleRouteBPoints(0.045), true), 0.6, { segments: 92, dashEvery: 9, opacity: 0.74 }));
}
...
function createRoad(from, to, width) {
  ...
  return createCurvedRoad(route, width, { segments: 20, dashEvery: 5 });
}
...
function createRocket(sectionId) {
  ...
  const body = addInteractive(new THREE.CylinderGeometry(0.31, 0.39, 3.05, 88, 8), materials.white, sectionId, group, 0, 2.06, 0);
  body.castShadow = true;
  const shoulder = addInteractive(new THREE.CylinderGeometry(0.36, 0.4, 0.34, 88), materials.white, sectionId, group, 0, 3.71, 0);
  shoulder.castShadow = true;
  addInteractive(new THREE.ConeGeometry(0.36, 0.98, 88), materials.white, sectionId, group, 0, 4.32, 0);
  addInteractive(new THREE.CylinderGeometry(0.365, 0.365, 0.2, 88), materials.rocketBlack, sectionId, group, 0, 2.98, 0);
  addInteractive(new THREE.CylinderGeometry(0.388, 0.388, 0.055, 88), materials.red, sectionId, group, 0, 1.18, 0);
  addInteractive(new THREE.CylinderGeometry(0.33, 0.37, 0.06, 88), materials.red, sectionId, group, 0, 3.58, 0);
  const nozzle = addInteractive(new THREE.ConeGeometry(0.31, 0.48, 48), materials.rocketBlack, sectionId, group, 0, 0.2, 0);
  ...
  [[-0.54, 0], [0.54, 0], [0, -0.54]].forEach(([x, z]) => {
    ...
    addInteractive(new THREE.CylinderGeometry(0.13, 0.155, 1.95, 54, 5), materials.white, sectionId, booster, 0, 0, 0);
    addInteractive(new THREE.ConeGeometry(0.145, 0.36, 54), materials.white, sectionId, booster, 0, 1.15, 0);
    const foot = addInteractive(new THREE.ConeGeometry(0.132, 0.28, 42), materials.rocketBlack, sectionId, booster, 0, -1.11, 0);
    foot.rotation.x = Math.PI;
    addInteractive(new THREE.CylinderGeometry(0.152, 0.152, 0.045, 54), materials.red, sectionId, booster, 0, 0.42, 0);
...
function createRover(accentColor, variant = 0) {
  ...
  wheelPositions.forEach(([x, z, front]) => {
    ...
    const tire = new THREE.Mesh(new THREE.CylinderGeometry(tireRadius, tireRadius, 0.32, 40), tireMat);
    ...
    const hub = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, 0.35, 28), edgeMat);
    hub.rotation.z = Math.PI / 2;
    axle.add(hub);
    const cap = new THREE.Mesh(new THREE.CylinderGeometry(0.13, 0.13, 0.365, 20), bodyMat);
    cap.rotation.z = Math.PI / 2;
    axle.add(cap);
    const ringMesh = new THREE.Mesh(new THREE.TorusGeometry(0.265, 0.01, 8, 40), redGlowMat.clone());
  ...
  const antennaBase = new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.04, 0.16, 10), edgeMat);
  ...
  group.add(antenna);
  const beacon = new THREE.Mesh(new THREE.SphereGeometry(0.045, 16, 10), redMat);
  beacon.position.set(0.46, 2.03, 1.18);
  group.add(beacon);
  const beaconLight = new THREE.PointLight(accentColor, 0.52, 2.7, 2.4);
